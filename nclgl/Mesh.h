...
public:
	...
	static Mesh* GenerateQuad(float scale,Vector3 origin, GLuint type);
	static Mesh* GenerateQuad2();
	static Mesh* GenerateQuad3();
	static Mesh* GenerateQuad4();
	struct SubMesh {
		int start;
		int count;
	};
	...
	void Draw(GLuint t);
	void Draw2(GLuint t, GLsizei instanceCount);
	...
	GLuint GetVBO(MeshBuffer bufferType) const {
		return bufferObject[bufferType];
	}
	GLuint GetVertexCount() const { return numVertices; }

	Vector2* getTextureCoords() const { return textureCoords; }
