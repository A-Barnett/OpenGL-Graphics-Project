...
void Mesh::Draw(GLuint	t) {
	glBindVertexArray(arrayObject);
	if (bufferObject[INDEX_BUFFER]) {
		glDrawElements(t, numIndices, GL_UNSIGNED_INT, 0);
	}
	else {
		glDrawArrays(t, 0, numVertices);
	}
	glBindVertexArray(0);
}
...
Mesh* Mesh::GenerateTriangle() {
	...
	m->vertices[0] = Vector3(0.0f, 0.45f, 0.0f);
	m->vertices[1] = Vector3(0.9f, -0.5f, 0.0f);
	m->vertices[2] = Vector3(-0.9f, -0.5f, 0.0f);
...
Mesh* Mesh::GenerateQuad(float scale, Vector3 origin, GLuint type) {
	Mesh* m = new Mesh();
	m->numVertices = 4;
	m->type = GL_PATCHES;

	m->vertices = new Vector3[m->numVertices];
	m->vertices[0] = (Vector3(-1.0f, 0.0f, 1.0f) * scale) + origin;   // Top-left in X-Z plane
	m->vertices[1] = (Vector3(-1.0f, 0.0f, -1.0f) * scale) + origin;  // Bottom-left in X-Z plane
	m->vertices[2] = (Vector3(1.0f, 0.0f, -1.0f) * scale) + origin;   // Bottom-right in X-Z plane
	m->vertices[3] = (Vector3(1.0f, 0.0f, 1.0f) * scale) + origin;    // Top-right in X-Z plane

	m->colours = new Vector4[m->numVertices];
	m->colours[0] = Vector4(1.0f, 0.0f, 0.0f, 1.0f);
	m->colours[1] = Vector4(0.0f, 1.0f, 0.0f, 1.0f);
	m->colours[2] = Vector4(0.0f, 0.0f, 1.0f, 1.0f);
	m->colours[3] = Vector4(0.0f, 1.0f, 1.0f, 1.0f);
...
Mesh* Mesh::GenerateQuad3() {
	Mesh* mesh = new Mesh();
	mesh->numVertices = 4;
	mesh->type = GL_TRIANGLE_STRIP;

	// Positions for a simple quad
	...
	// Texture coordinates (covering full range [0,0] to [1,1])
	mesh->textureCoords = new Vector2[mesh->numVertices]{
		Vector2(0.0f, 1.0f), // Top-left
		Vector2(0.0f, 0.0f), // Top-right
	...
	for (int i = 0; i < 4; ++i) {
		...
		mesh -> tangents[i] = Vector4(1.0f, 0.0f, 0.0f, 1.0f); 
	}
	// Specify indices if needed for triangle formation
	mesh->BufferData();

	return mesh;
}

Mesh* Mesh::GenerateQuad4() {
	Mesh* mesh = new Mesh();
	mesh->numVertices = 4;
	mesh->type = GL_TRIANGLE_STRIP;
	...
	// Positions for a simple quad
	mesh->vertices = new Vector3[mesh->numVertices]{
		Vector3(-0.15f, 4.0f, 0.0f),  // Top-left
		Vector3(0.15f, 4.0f, 0.0f),   // Top-right
		Vector3(-0.15f, -4.0f, 0.0f), // Bottom-left
		Vector3(0.15f, -4.0f, 0.0f)   // Bottom-right
	};
...
void Mesh::Draw2(GLuint t, GLsizei instanceCount) {
	glBindVertexArray(arrayObject);
	if (bufferObject[INDEX_BUFFER]) {
		glDrawElementsInstanced(t, numIndices, GL_UNSIGNED_INT, 0, instanceCount);
	}
	else {
		glDrawArraysInstanced(t, 0, numVertices, instanceCount);
	}
	glBindVertexArray(0);
}